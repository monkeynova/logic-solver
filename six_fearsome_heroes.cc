...
puzzle::Solution SixFearsomeHeroes::GetSolution() const {
  ...
  // Riker: hero=Picard fear=Worf trid=3 fizzbin=5
  entries.emplace_back(RIKER, std::vector<int>{PICARD, WORF, 3, 5},
                       entry_descriptor());

  // Troi: hero=Worf fear=Riker trid=1 fizzbin=4
  entries.emplace_back(TROI, std::vector<int>{WORF, RIKER, 1, 4},
                       entry_descriptor());

  // Geordi: hero=Riker fear=Picard trid=2 fizzbin=3
  entries.emplace_back(GEORDI, std::vector<int>{RIKER, PICARD, 2, 3},
                       entry_descriptor());

  // Data: hero=Troi fear=Geordi trid=4 fizzbin=1
  entries.emplace_back(DATA, std::vector<int>{TROI, GEORDI, 4, 1},
                       entry_descriptor());

  // Worf: hero=Geordi fear=Data trid=6 fizzbin=6
  entries.emplace_back(WORF, std::vector<int>{GEORDI, DATA, 6, 6},
                       entry_descriptor());
