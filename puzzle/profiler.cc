...
class StructTimevalProfiler : public Profiler {
 ...
 private:
  bool Done() override { return test_calls() > FLAGS_puzzle_max_profile_calls; }
  ...
  bool NotePositionImpl(double position, double count) override {
    ...
    std::cout << std::setprecision(3) << "\033[1K\rTrying "
              << (100 * position / count) << "%, effective=" << qps / 1000
              << "Kqps true=" << (test_calls() / full_delta / 1000) << "Kqps"
              << std::flush;
    last_ = now;
  ...
  void NoteFinish() override { gettimeofday(&last_, nullptr); }

  double Seconds() override {
    return last_.tv_sec - start_.tv_sec +
           1e-6 * (last_.tv_usec - start_.tv_usec);
  }
